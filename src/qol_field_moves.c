...
void ReturnToFieldOrBagFromFlyTool(void) //originally 32bit, idk why
{
...
bool32 PartyHasMonLearnsKnowsFieldMove(u16 itemId)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        monCanLearnTM = CanTeachMove(mon,itemId);
        if ((monCanLearnTM) == ALREADY_KNOWS_MOVE
                || (monCanLearnTM) == CAN_LEARN_MOVE)
            return SetMonResultVariables(i,species);

        // for (i = 0; i < TUTOR_MOVE_COUNT; i++) should be uncessary as above should cover tutor I believe
        // {
