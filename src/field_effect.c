...
void Task_UseWaterfall(u8 taskId) //static
{
    while (sWaterfallFieldEffectFuncs[gTasks[taskId].tState](&gTasks[taskId], &gObjectEvents[gPlayerAvatar.objectEventId]));
}

bool8 WaterfallFieldEffect_Init(struct Task *task, struct ObjectEvent *objectEvent) //static
{
    LockPlayerFieldControls();
    gPlayerAvatar.preventStep = TRUE;
    task->tState++;
...
bool8 WaterfallFieldEffect_RideUp(struct Task *task, struct ObjectEvent *objectEvent) //static
{
...
bool8 WaterfallFieldEffect_ContinueRideOrEnd(struct Task *task, struct ObjectEvent *objectEvent) //static
{
    if (!ObjectEventClearHeldMovementIfFinished(objectEvent))
        return FALSE;
...
bool8 DiveFieldEffect_Init(struct Task *task) //static
{
    gPlayerAvatar.preventStep = TRUE;
    task->data[0]++;
    return FALSE;
...
bool8 DiveFieldEffect_TryWarp(struct Task *task) //static
{
...
void Task_SurfFieldEffect(u8 taskId) //static
{
    sSurfFieldEffectFuncs[gTasks[taskId].tState](&gTasks[taskId]);
}

void SurfFieldEffect_Init(struct Task *task) //static
{
    LockPlayerFieldControls();
    FreezeObjectEvents();
    gPlayerAvatar.preventStep = TRUE;
...
void SurfFieldEffect_JumpOnSurfBlob(struct Task *task) //static
{
    struct ObjectEvent *objectEvent;
    if (!FieldEffectActiveListContains(FLDEFF_FIELD_MOVE_SHOW_MON))
    {
...
void SurfFieldEffect_End(struct Task *task) //static
{
    struct ObjectEvent *objectEvent;
    objectEvent = &gObjectEvents[gPlayerAvatar.objectEventId];
    if (ObjectEventClearHeldMovementIfFinished(objectEvent))
